...
export default function DQAnimationSpriteImage({ surface, direction, width, height }: DQAnimationSpriteImageProps): ReactNode {
  const timer = useTimer();
  const [position, setPosition] = useState<[number, number]>();

  useEffect(() => {
    const { x, y } = getDQAnimationSpritePosition(surface, direction, Number(timer % 2n));
